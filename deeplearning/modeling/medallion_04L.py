...
class ModelConfig(config_base.ModelConfig):
  ...
  def __init__(self, learning_rate=5e-5, vocab_size=512, num_labels=2, max_sentence_len=64, dist_bucket_boundaries=None, hidden_size=768,
    ...
    """
    super().__init__()
    if dist_bucket_boundaries is None:
        dist_bucket_boundaries = [float(i) for i in range(0, 500) if i % 5 == 0]
    self.learning_rate = learning_rate

    self.num_labels = num_labels
    self.vocab_size = vocab_size
    self.max_sentence_len = max_sentence_len
    self.dist_bucket_boundaries = dist_bucket_boundaries
...
class ModelBuilder(modeling_base.ModelBuilder):
    def __init__(self):
        ...
        self.fea_model_config = ModelConfig(max_sentence_len=4*15*2, hidden_size=128,
                                            num_hidden_layers=1, num_attention_heads=4, intermediate_size=128)
        self.seq_model_config = ModelConfig(max_sentence_len=64, hidden_size=512,
                                            num_hidden_layers=8, num_attention_heads=8, intermediate_size=1024)

    def get_name_to_features(self, with_labels=True):
        model_conf = self.seq_model_config

        name_to_features = {"name": tf.io.FixedLenFeature([], tf.string),
                            "datetime": tf.io.FixedLenFeature([], tf.string),
                            "input_status": tf.io.FixedLenFeature([3840], tf.float32),
                            "next_6_close": tf.io.FixedLenFeature([6], tf.float32)}

        # if with_labels:
        #     name_to_features["next_6_close"] = tf.io.FixedLenFeature([6], tf.float32)
    ...
    def build_model(self, features, profit_labels, is_training, with_labels=True):
        ...
        model_config = self.seq_model_config
        use_one_hot_embeddings = False

        self.name = features["name"]
        self.datetime = features["datetime"]

        input_state = features["input_status"]
        # input_state = tf.where(tf.is_nan(input_state), tf.zeros_like(input_state), input_state)
        # profit_labels = None
        # profit_loss_weights = None
        # next_6_close = None
        next_6_close = features["next_6_close"]
        ...
        new_feature_size = security_size * security_fea_size * 2
        input_state = tf.clip_by_norm(input_state, 1, axes=[1])
        input_state = tf.reshape(input_state, [batch_size, seq_length, new_feature_size])
        ...
        # pretrain loss
        # batch_mean_loss = masked_lm_loss + loss
        # batch_sample_loss = masked_lm_batch_sample_loss + batch_sample_loss

        # finetune loss
        # batch_mean_loss = loss
        # batch_sample_loss = batch_sample_loss
    ...
    def get_predict_ops(self):
        return {
            ...
            "batch_sample_loss": self.batch_sample_loss,
        }
  ...
  def __init__(self,
    ...
    """Constructor for BertModel.
          ...
          Args:
            ...
            input_state: int32 Tensor of shape [batch_size, seq_length].
    ...
    input_shape = get_shape_list(input_state, expected_rank=3)
    batch_size = input_shape[0]
    seq_length = input_shape[1]
    state_size = input_shape[2]
...
def embedding_lookup(input_state,
                     state_size,
                     embedding_size=128,
  ...
  """Looks up words embeddings for id tensor.
    ...
    Args:
      input_state: int32 Tensor of shape [batch_size, seq_length] containing word
        ids.
  ...
  embedding_table = tf.get_variable(
      ...
      shape=[state_size, embedding_size],
      initializer=create_initializer(initializer_range))

  # flat_input_ids = tf.reshape(input_state, [-1])

  output = tf.matmul(input_state, embedding_table)
...
def embedding_sentence_dist(input_tensor,
                            sentence_ids,
                            max_sentence_len,
                            tts_dist,
    ...
    sentence_embed_table = tf.get_variable(
    ...
    # tts_dist embedding
    dist_embed_table = tf.get_variable(
        name="tts_dist_embedding",
...
def get_masked_lm_output(bert_config: ModelConfig, input_tensor, output_weights, positions,
  ...
  with tf.variable_scope("cls/predictions"):
    ...
    # padding predictions.
    per_batch_sample_loss = -tf.reduce_sum(log_probs * one_hot_labels, axis=[-1])
    numerator = tf.reduce_sum(label_weights * per_batch_sample_loss)
...
def get_next_sentence_output(bert_config: ModelConfig, input_tensor, labels, loss_weights):
