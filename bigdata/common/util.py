...
class HdfsUtil(object):
    ...
    @staticmethod
    def check_success(hdfs_dir, overwrite=False):
        hdfs_success_file = "{}/_SUCCESS".format(hdfs_dir)
        cmd = "hdfs dfs -test -e {}".format(hdfs_success_file)
        status = os.system(cmd)
        if not overwrite and int(status) == 0:
            logger.info("输出路径数据已存在，并且 overwrite: {}\ntfrecord_output_file: {}".format(overwrite, hdfs_dir))
            print("....... 程序退出 ......")
            exit(0)
