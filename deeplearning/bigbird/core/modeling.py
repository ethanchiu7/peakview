...
class BertModel(tf.keras.layers.Layer):
  ...
  def call(self,
           ...
           token_type_ids=None,
           training=None,
           inputs_is_embedding=False,
           inputs_mask=None):
      """Constructor for BertModel.

      Args:
        inputs: int32 Tensor of shape [batch_size, seq_length].
      ...
      Raises:
        ValueError: The config is invalid or one of the input tensor shapes
          is invalid.
      """
      if inputs_is_embedding and inputs_mask is not None:
          return self.input_embedding_to_pooled_output(inputs, inputs_mask, training)
      # pad if needed
      ...
      input_embedding = self.embeder(inputs,
                                     self.params["max_encoder_length"],
                                     token_type_ids=token_type_ids,
                                     training=training)

      # Generate mask.
      input_mask = tf.where(inputs > 0,
                            tf.ones_like(inputs), tf.zeros_like(inputs))

      # Run the stacked transformer.
      sequence_output = self.encoder(input_embedding, input_mask, training)
  ...
  def input_embedding_to_pooled_output(self, input_embedding, input_mask, training):
      # Run the stacked transformer.
      sequence_output = self.encoder(input_embedding, input_mask, training)
