...
class ModelConfig(config_base.ModelConfig):
    ...
    def __init__(self, learning_rate=5e-5, vocab_size=512, num_labels=2, max_sentence_len=64,
                 dist_bucket_boundaries=None, hidden_size=768,
...
class ModelBuilder(modeling_base.ModelBuilder):
    ...
    def build_model(self, features, profit_labels, is_training, with_labels=True):
        ...
        input_state_td = tf.concat([tf.zeros(shape=[batch_size, 1, security_size, security_fea_size]), input_state_td],
                                   axis=1)

        # cross features
        # price_features = input_status[:, :, :, :]

        input_state = tf.concat([input_state_td], axis=-1)
        # input_state = tf.clip_by_norm(input_state, 1, axes=[1])
        input_state = tf.reshape(input_state, [batch_size, seq_length, security_size * security_fea_size])
