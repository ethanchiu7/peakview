...
def get_estimator(config, model_fn, keep_checkpoint_max=10):
  ...
  # GPU: Estimator needs per bigbird batch size
  # TPU: Estimator needs total batch size, i.e. num_cores * per bigbird batch size
  ...
  if config["use_tpu"]:
    ...
    with tf.Session(cluster_resolver.master(),
  ...
  if config["init_checkpoint"]:
    ckpt_var_list = tf.train.list_variables(config["init_checkpoint"])
    ckpt_var_list = {
